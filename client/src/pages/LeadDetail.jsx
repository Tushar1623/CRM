...
import { Phone, MessageCircle, Calendar, Receipt, ChevronLeft, Clock, FileText, CheckSquare, CarFront, CheckCircle2, X } from 'lucide-react';
import api from '../api';
...
function LeadDetail() {
  ...
  const handleCreateFollowup = async (e) => {
    ...
    try {
      ...
      await api.post('/api/followups', {
        ...
        type: followupType,
        notes: followupNotes || 'Follow-up call with customer',
  ...
  const handleCreateTestDrive = async (e) => {
    ...
    try {
      await api.post('/api/test-drives', {
        ...
        location: e.target.location?.value || 'Showroom',
        status: 'Scheduled'
      });
      setShowTestDrive(false);
      fetchLeadDetails();
      window.dispatchEvent(new CustomEvent('crm-toast', { detail: 'Test drive scheduled!' }));
    } catch (err) {
  ...
  if (loading) return <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-secondary)' }}>Loading Lead Profile...</div>;
  if (!data || !data.lead) return <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-secondary)' }}>Lead not found!</div>;

  const { lead, activities, followups, testDrives, deals } = data;
  const cust = lead.customer_id;
  const phoneClean = cust?.phone ? String(cust.phone).replace(/[^0-9]/g, '').slice(-10) : '';
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          ...
          <h2 style={{ fontSize: '1.4rem', fontWeight: '700', margin: 0 }}>Lead Profile #{lead.lead_number || ''}</h2>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '2rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div className="glass-panel" style={{ padding: '1.75rem' }}>
            <h3 style={{ margin: '0 0 1.25rem 0', fontSize: '1.05rem', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '0.5rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.75rem' }}>
              <FileText size={18} color="var(--text-secondary)" /> Enquiry & Buying Requirements
            </h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
              ...
              <div>
                <p style={{ margin: '0 0 0.25rem 0', fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: '600' }}>Lead Source</p>
                <p style={{ margin: 0, fontSize: '0.95rem' }}>{lead.source || 'Walk-in'}</p>
              </div>
              <div>
                <p style={{ margin: '0 0 0.25rem 0', fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: '600' }}>Assigned Salesperson</p>
                <p style={{ margin: 0, fontSize: '0.95rem' }}>{lead.assigned_to?.name || lead.assigned_to_name || (typeof lead.assigned_to === 'string' ? lead.assigned_to : 'Amit Sharma')}</p>
              </div>
            </div>
            {lead.notes && (
              <div style={{ marginTop: '1.25rem', paddingTop: '1rem', borderTop: '1px solid var(--border-color)' }}>
                <p style={{ margin: '0 0 0.25rem 0', fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: '600' }}>Notes</p>
                <p style={{ margin: 0, fontSize: '0.88rem', color: 'var(--text-primary)' }}>{lead.notes}</p>
              </div>
            )}
          </div>
          ...
          {deals && deals.length > 0 && (
            <div className="glass-panel" style={{ padding: '1.75rem' }}>
              <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.05rem', fontWeight: '700', color: '#10b981' }}>Deals & Bookings</h3>
                  ...
                  <div key={d._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.85rem 1rem', background: 'var(--bg-dark)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '0.2rem' }}>
                        Selling Price: ₹{d.selling_price?.toLocaleString()} • Token: ₹{d.booking_amount?.toLocaleString()}
                      </div>
                    </div>
          ...
          {/* Test Drives for this Lead */}
          {testDrives && testDrives.length > 0 && (
            <div className="glass-panel" style={{ padding: '1.75rem' }}>
              <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.05rem', fontWeight: '700' }}>Test Drives</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {testDrives.map(td => (
                  <div key={td._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.85rem 1rem', background: 'var(--bg-dark)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                    <div>
                      <strong style={{ fontSize: '0.92rem' }}>{td.vehicle_id?.brand} {td.vehicle_id?.model || td.car_name}</strong>
                      <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '0.2rem' }}>
                        {new Date(td.date).toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' })} at {td.time}
                      </div>
                    </div>
        ...
        {/* Right Column: Scheduled Follow-ups & Chronological Audit Timeline */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          {/* Activity Timeline */}
          <div className="glass-panel" style={{ padding: '1.75rem' }}>
            <h3 style={{ margin: '0 0 1rem 0', fontSize: '1.05rem', fontWeight: '700' }}>Dealership Activity Audit</h3>
        ...
        <div className="modal-overlay" onClick={() => setShowFollowup(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ padding: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: '700' }}>Schedule Follow-up for {cust?.name}</h3>
            ...
            <form onSubmit={handleCreateFollowup} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: 'var(--text-secondary)', marginBottom: '0.4rem' }}>Follow-up Channel *</label>
              ...
              <div>
                ...
                <input 
                  className="premium-input" 
                  placeholder="e.g. Follow up regarding exchange valuation"
                  value={followupNotes}
      ...
      {showTestDrive && (
        <div className="modal-overlay" onClick={() => setShowTestDrive(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ padding: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: '700' }}>Schedule Test Drive for {cust?.name}</h3>
              <button onClick={() => setShowTestDrive(false)} style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }}>
                <X size={20} />
              </button>
            </div>
            <form onSubmit={handleCreateTestDrive} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                ...
                <input name="location" defaultValue="Dealership Showroom" className="premium-input" />
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '1rem' }}>
      ...
      {showDeal && (
        <div className="modal-overlay" onClick={() => setShowDeal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ padding: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: '700' }}>Convert Lead to Deal</h3>
            ...
            <form onSubmit={handleCreateDeal} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                ...
                {vehicles.length > 0 ? (
                  <select className="premium-input" value={selectedVehicle} onChange={e => setSelectedVehicle(e.target.value)}>
                    {vehicles.map(v => (
                      <option key={v._id} value={v._id}>{v.brand} {v.model} - ₹{(v.selling_price || v.asking_price)?.toLocaleString()}</option>
                    ))}
                  </select>
                ) : (
                  <input 
